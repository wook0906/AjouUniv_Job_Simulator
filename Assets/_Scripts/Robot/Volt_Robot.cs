...
public class Volt_Robot : MonoBehaviour
{
    ...
    public IEnumerator Synchronization(RobotData data)
    {
        ...
        transform.rotation = Quaternion.LookRotation(Volt_Utils.StringToVector3(data.lookDirection), Vector3.up);
    ...
    private void OnDestroy()
    {
        ...
        SimulationObserver.Instance.OffBehaviorFlag(playerInfo.playerNumber);
    ...
    public void ForcedKillRobot()
    {
        ...
        //    Volt_GameManager.S.AmargeddonCount = 0;
        //}
        if (fsm.Owner.playerInfo.playerNumber == GameController.instance.gameData.AmargeddonPlayer)
        {
            GameController.instance.gameData.AmargeddonPlayer = 0;
    ...
    public void DetectRobot()
    {
        ...
        if (IsOnVPCoin(Volt_ArenaSetter.S.GetTile(transform.position)))
        {
            ...
            if (GameController.instance.gameData.isKillbotBehaviourOff)
                GameController.instance.gameData.behaviours[playerInfo.playerNumber].BehaviourType = BehaviourType.None;
        ...
        else if (targetTile == null)
        {
            ...
            int behaviourPoints = GameController.instance.gameData.KillbotBehaviourPoints;//Random.Range(2, 4);//2; //추후 1~3 랜덤하게 설정
