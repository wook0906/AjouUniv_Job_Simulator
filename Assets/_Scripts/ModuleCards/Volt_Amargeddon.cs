            ...
            other.CompareTag("Killbot") && !isTriggered)
        {
            ...
            foreach (var item in targetTiles)
            {
                ...
                if (robot != null)
                {
                    ...
                    robot.GetDamage(new AttackInfo(GameController.instance.gameData.AmargeddonPlayer, 1, CameraShakeType.Amargeddon, Volt_PlayerManager.S.GetPlayerByPlayerNumber(GameController.instance.gameData.AmargeddonPlayer).GetHitEffect(), Card.AMARGEDDON));
