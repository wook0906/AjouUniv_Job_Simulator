...
{ 
    ...
    [SerializeField]
    Define.Phase curPhase;
    public Define.Phase pCurPhase
    {
        get { return curPhase; }
        set { curPhase = value; }
    }
    ...
    {
        Volt_ModuleDeck.S.SetModuleDeckSettingData((Define.MapType)mapType);
        StartCoroutine(ArenaSetup(mapType));
    }

    IEnumerator ArenaSetup(int mapType)
    {
        //curPhase = Define.Phase.fieldSetup;
        ...
        else
        {
            switch (mapType)
            {
                case 0:
                    sceneName = "TwinCity";
                    this.mapType = Define.MapType.TwinCity;
                    break;
                case 1:
                    sceneName = "Rome";
                    this.mapType = Define.MapType.Rome;
                    break;
                case 2:
                    sceneName = "Ruhrgebiet";
                    this.mapType = Define.MapType.Ruhrgebiet;
                    break;
                case 3:
                    sceneName = "Tokyo";
                    this.mapType = Define.MapType.Tokyo;
    ...
    public void ItemSetupStart(int vpIdx, int repairKitIdx, int moduleIdx, Card cardType)
    {
        ...
        curPhase = Define.Phase.ItemSetup;
    ...
    IEnumerator ItemSetup(int vpIdx, int repairKitIdx, int moduleIdx, Card cardType)
    {
        ...
        if (mapType == Define.MapType.Ruhrgebiet)
        {
    ...
    public void PlaceRobotStart() //서버에서 로봇 배치 타이밍을 알려줄 때 호출 될 수 있도록 한다.
    {
        ...
        //curPhase = Define.Phase.robotSetup;
    ...
    IEnumerator WaitPlaceRobot()
    {
        ...
        if (!IsTutorialMode)
        {
            if (!Volt_PlayerManager.S.I.playerRobot)
            {
                if (IsFullStartingTiles() && pCurPhase == Define.Phase.PlaceRobot)
                {
        ...
        {
            if (Volt_PlayerManager.S.I.PlayerType == PlayerType.HOSTPLAYER && !IsAllUserHasRobot() && pCurPhase == Define.Phase.PlaceRobot)
            {
                foreach (var item in Volt_PlayerManager.S.GetPlayers())
                {
        ...
        if (!IsAllUserHasRobot() && pCurPhase == Define.Phase.PlaceRobot)
        {
            PlaceRobot phase = GameController.instance.CurrentPhase as PlaceRobot;
            if (Volt_PlayerManager.S.I.PlayerType == PlayerType.HOSTPLAYER)
            {
                foreach (var item in Volt_PlayerManager.S.GetPlayers()) //나말고 다른애들이 로봇이 없으니까 자동배치 시켜주고.
                {
                    ...
                    {
                        //item.AutoRobotPlace();
                        phase.AutoRobotSetup(item.playerNumber);
                    }
            ...
            if (!Volt_PlayerManager.S.I.playerRobot) //내꺼 없으면 자동생성하고
            {
                ...
                phase.AutoRobotSetup(Volt_PlayerManager.S.I.playerNumber);
            }
        ...
        if (IsTutorialMode || IsTrainingMode)
        {
            ...
            if (curPhase == Define.Phase.GameOver) yield break;
    ...
    IEnumerator PlayerRobotPlace()
    {
        ...
        yield return new WaitUntil(() => IsAllPlayerWaitForRobotPlace() && (pCurPhase == Define.Phase.PlaceRobot || pCurPhase == Define.Phase.Synchronization));
    ...
    public void SelectBehaviourTypeStart()
    {
        ...
        curPhase = Define.Phase.SelectBehaviour;
    ...
    public IEnumerator WaitCameraAnimation(bool rewind)
    {
        ...
        if (!rewind)
        {
            ...
            while (t <= 1f)
            {
                ...
                if (curPhase == Define.Phase.WaitSync)
                {
                    camRoot.CamInit();
    ...
    public void SimulationStart(int option,int option2, int option3, int option4)
    {
        ...
        curPhase = Define.Phase.Simulation;
    ...
    public IEnumerator Simulate(int option, int option2, int option3, int option4)
    {
        ...
        while (behaviourStack.Count > 0)
        {
            ...
            if (AmargeddonCount > 0 && AmargeddonPlayer != 0)
            {
                ...
                //Debug.Log(AmargeddonPlayer +" : " + AmargeddonCount + i"번 남았으");
    ...
    {
        curPhase = Define.Phase.SuddenDeath;
        if(RoundNumber == 10)
        {
    ...
    {
        if (curPhase == Define.Phase.WaitSync) return;
    ...
    public void SendCurrentGameData()
    {
        ...
        //curPhase = Define.Phase.synchronization;
    }
    ...
    {
        yield return new WaitUntil(() => IsAllRobotIdleState() && SimulationObserver.Instance.IsAllRobotBehaviourFlagOff());
        yield return new WaitUntil(() => curPhase == Define.Phase.Synchronization || curPhase == Define.Phase.WaitSync);
    ...
    {
        curPhase = Define.Phase.GameOver;
