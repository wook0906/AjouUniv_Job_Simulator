...
public class Volt_ArenaSetter : MonoBehaviour
{
    ...
    public StartingTileSetData playerStartTileIdxData;
    ...
    public IEnumerator SetupField()
    {
        ...
        //GameController.instance.PlayerSetupStart();
        //switch (Volt_GameManager.S.mapType)
        switch (GameController.instance.gameData.mapType)
        {
            case Define.MapType.TwinCity:
                break;
            case Define.MapType.Rome:
                ballistaLaunchPoints = GameObject.FindGameObjectsWithTag("Ballista").OrderBy(x => int.Parse(x.name)).ToArray();

                foreach (var item in ballistaLaunchPoints)
                {
