    ...
    private void OnClickMapButton(UIButton button, Transform mapButton)
    {
        ...
        switch (button.name)
        {
            case "TwinCity_Btn":
                if(isSelectTwincity)
                {
                    PlayerPrefs.SetInt("SELECTED_MAP", (int)Define.MapType.TwinCity);
            ...
            case "Rome_Btn":
                if (isSelectRome)
                {
                    PlayerPrefs.SetInt("SELECTED_MAP", (int)Define.MapType.Rome);
                    PlayerPrefs.SetInt("Volt_TrainingMode", 1);
                    isStartGame = true;
                }
                else
            ...
            case "Ruhrgebiet_Btn":
                ...
                {
                    PlayerPrefs.SetInt("SELECTED_MAP", (int)Define.MapType.Ruhrgebiet);
                    PlayerPrefs.SetInt("Volt_TrainingMode", 1);
                    isStartGame = true;
                }
                else
            ...
            case "Tokyo_Btn":
                if (isSelectTokyo)
                {
                    PlayerPrefs.SetInt("SELECTED_MAP", (int)Define.MapType.Tokyo);
                    PlayerPrefs.SetInt("Volt_TrainingMode", 1);
                    isStartGame = true;
                }
                else
