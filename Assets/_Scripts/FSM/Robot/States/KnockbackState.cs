...
public class KnockbackState : StateBase
{
    ...
    public override void OnEnterState(StateMachine fsm)
    {
        ...
        //if(fsm.Owner.lastAttackPlayer == 0 && Volt_GameManager.S.pCurPhase != Define.Phase.SuddenDeath)
        if(fsm.Owner.lastAttackPlayer == 0 && GameController.instance.CurrentPhase.type != Define.Phase.SuddenDeath)
            fsm.Owner.lastAttackPlayer = SimulationObserver.Instance.currentPusher.playerInfo.playerNumber;
