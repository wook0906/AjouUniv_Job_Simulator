...
public class DeadState : StateBase
{
    ...
    private void HandleDeathEachGamePhase(StateMachine fsm)
    {
        ...
        switch (GameController.instance.CurrentPhase.type)
        {
            case Define.Phase.SuddenDeath:
                ...
                break;
            case Define.Phase.ResolutionTurn:
                type = DeathType.Trap;
                break;
            default:
                if (IsSelfKill(fsm))
    ...
    private void ShowMessage(DeathType type, StateMachine fsm)
    {
        ...
        switch (type)
        {
            ...
            case DeathType.KilledByOpponent:
                ...
                {
                    Volt_GMUI.S.Create2DMsg(MSG2DEventType.DeadToPlayer, SimulationObserver.Instance.currentPusher.playerInfo.playerNumber);
                }
    ...
    public override void OnEnterState(StateMachine fsm)
    {
        ...
        fsm.isDead = true;
        fsm.elapsedTime = 0f;
        SimulationObserver.Instance.OnBehaviorFlag(fsm.Owner.playerInfo.playerNumber);
    ...
    public override void Action(StateMachine fsm, float deltaTime)
    {
        ...
        SimulationObserver.Instance.OffBehaviorFlag(fsm.Owner.playerInfo.playerNumber);
