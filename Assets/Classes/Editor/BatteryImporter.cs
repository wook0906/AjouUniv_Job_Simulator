...
public class BatteryImporter : AssetPostprocessor {
	private static readonly string filePath = "Assets/Resources/Data/Battery.xls";
	private static readonly string exportPath = "Assets/Resources/Data/Battery.json";
	private static readonly string[] sheetNames = { "BatteryShop", };
	...
	static void OnPostprocessAllAssets (string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
	{
		...
		foreach (string asset in importedAssets) {
			...
			BatteryShopTable data = new BatteryShopTable ();
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					BatteryShopTable.Sheet s = new BatteryShopTable.Sheet ();
					s.name = sheetName;
